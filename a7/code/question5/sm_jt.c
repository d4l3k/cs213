...
int exec() {
  int cont = 1;
  int addr, val;
  void *jumps[] = {&&LDI, &&LDO, &&LD, &&STO, &&ST, &&DEFAULT, &&ALU, &&SH, &&BR, &&BEG, &&BGT, &&JI, &&JO, &&JDO, &&JDI, &&HALT};
  void *aluJumps[] = {&&MOV, &&ADD, &&AND, &&INC, &&INCA, &&DEC, &&DECA, &&NOT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&ALU_DEFAULT, &&GPC};
  if (insOpCode < 0 || insOpCode > 0xf) {
    goto DEFAULT;
  }
  goto *jumps[insOpCode];
LDI:
...
ST:
  ...
  mem [addr+3] = val       & 0xff;
  goto DONE;
ALU: //case 0x6: // ALU ................... 6-sd
  if (insOp0 < 0 || insOp0 > 0xf) {
    goto ALU_DEFAULT;
  }
  goto *aluJumps[insOp0];
MOV: //case 0x0: // mov rs, rd ........ 60sd
  reg [insOp2] = reg [insOp1];
  goto ALU_DONE;
ADD: //case 0x1: // add rs, rd ........ 61sd
  reg [insOp2] = reg [insOp1] + reg [insOp2];
  goto ALU_DONE;
AND: //case 0x2: // and rs, rd ........ 62sd
  reg [insOp2] = reg [insOp1] & reg [insOp2];
  goto ALU_DONE;
INC: //case 0x3: // inc rr ............ 63-r
  reg [insOp2] = reg [insOp2] + 1;
  goto ALU_DONE;
INCA: //case 0x4: // inca rr ........... 64-r
  reg [insOp2] = reg [insOp2] + 4;
  goto ALU_DONE;
DEC: //case 0x5: // dec rr ............ 65-r
  reg [insOp2] = reg [insOp2] - 1;
  goto ALU_DONE;
DECA: //case 0x6: // deca rr ........... 66-r
  reg [insOp2] = reg [insOp2] -4;
  goto ALU_DONE;
NOT: //case 0x7: // not ............... 67-r
  reg [insOp2] = ~ reg [insOp2];
  goto ALU_DONE;
GPC: //case 0xf: // gpc ............... 6f-r
...
ALU_DONE:
  goto DONE;
SH: //case 0x7: // sh? $i,rd ............. 7dii
  if (insOpImm > 0)
    reg [insOp0] = reg [insOp0] << insOpImm;
  else
    reg [insOp0] = reg [insOp0] >> -insOpImm;
  goto DONE;
BR: //case 0x8: // br o .................. 8-oo
  pc += insOpImm << 1;
  goto DONE;
BEG: //case 0x9: // beq rs, o ............. 9roo
  if (reg [insOp0] == 0)
    pc += insOpImm << 1;
  goto DONE;
BGT: //case 0xa: // bgt rs, o .............. aroo
  if (reg [insOp0] > 0)
    pc += insOpImm << 1;
  goto DONE;
JI: //case 0xb: // j i ................... b--- iiii iiii
  pc = insOpExt;
  goto DONE;
JO: //case 0xc: // j o(rr) ............... croo
  pc = (((unsigned short) insOpImm) << 1) + reg [insOp0];
  goto DONE;
JDO: //case 0xd: // j *o(rr) .............. droo
  // NYI
  goto DONE;
JDI: //case 0xe: // j*(rr,ri,4) ............. eri-
  // NYI
  goto DONE;
HALT: //case 0xf: // halt or nop ............. f?--
  if (insOp0 == 0)
    cont = 0;
  goto DONE;
DEFAULT: //default:
